...
public class BusinessDaoImpl extends BaseDaoImpl implements IBusinessDao {
	...
	@Override
	public List<Map<String, Object>> queryBusinessConsumerHistory(Locale locale, String businessId, String handler) {

		// @formatter:off
		/**
		SELECT
			business_consumer.business_id,
			business_consumer.id,
			business_consumer.consumer_id,
			business_consumer.consume_times,
			DATE_FORMAT(
				business_consumer.last_consume_time,
				'%Y/%m/%d %H:%i:%s'
			) AS last_consume_time,
			UNIX_TIMESTAMP(
				business_consumer.last_consume_time
			) AS times,
			TIMESTAMPDIFF(
			...
			business_consumer.scene_id,
			business_consumer.consume_code,
			business_consumer.`handler`,
			DATE_FORMAT(
				business_consumer.handle_date_time,
				'%Y/%m/%d %H:%i:%s'
			) AS handle_date_time,
			UNIX_TIMESTAMP(business_consumer.handle_date_time) AS handle_times,
			TIMESTAMPDIFF(
				SECOND,
				business_consumer.handle_date_time,
				NOW()
		...
		FROM
			business_consumer
		INNER JOIN qrcode ON business_consumer.scene_id = qrcode.scene_id
		INNER JOIN subscriber ON business_consumer.consumer_id = subscriber.user_name
		WHERE
			business_consumer.`status` = 1
		AND business_consumer.`handler` = 'okPUDtzjrogWsWdsI_Fz39VNbgBc'
		AND business_consumer.business_id = 'okPUDtzjrogWsWdsI_Fz39VNbgBc'**/
		// @formatter:on
		...
		sqlSb.append("	business_consumer.scene_id,\n");
		sqlSb.append("	business_consumer.consume_code,\n");
		sqlSb.append("	business_consumer.`handler`,\n");
		sqlSb.append("	DATE_FORMAT(\n");
		sqlSb.append("		business_consumer.handle_date_time,\n");
		sqlSb.append("		'%Y/%m/%d %H:%i:%s'\n");
		sqlSb.append("	) AS handle_date_time,\n");
		sqlSb.append("	UNIX_TIMESTAMP(business_consumer.handle_date_time) AS handle_times,\n");
		sqlSb.append("	TIMESTAMPDIFF(\n");
		sqlSb.append("		SECOND,\n");
		sqlSb.append("		business_consumer.handle_date_time,\n");
		sqlSb.append("		NOW()\n");
		...
		sqlSb.append("LEFT JOIN subscriber ON business_consumer.consumer_id = subscriber.user_name\n");
		sqlSb.append("WHERE\n");
		sqlSb.append("	business_consumer.`status` = 1\n");
		sqlSb.append("AND business_consumer.`handler` = ?\n");
		sqlSb.append("AND business_consumer.business_id = ?\n");
		sqlSb.append("ORDER BY\n");
		sqlSb.append("	business_consumer.handle_date_time DESC\n");

		return getMapList(sqlSb, handler, businessId);
	}

	@Override
	public List<Map<String, Object>> queryConsumerRequestHistory(Locale locale, String businessId, String handler) {
		// @formatter:off
		/**
		SELECT
			service.id,
			...
			UNIX_TIMESTAMP(service.date_time) AS times,
			TIMESTAMPDIFF(
				SECOND,
				service.date_time,
				NOW()
			...
			service.scene_id,
			DATE_FORMAT(
				service.handle_date_time,
				'%Y/%m/%d %H:%i:%s'
			) AS handle_date_time,
			UNIX_TIMESTAMP(service.handle_date_time) AS handle_times,
			TIMESTAMPDIFF(
				SECOND,
				service.handle_date_time,
				NOW()
		...
		FROM
			service
		INNER JOIN qrcode ON service.scene_id = qrcode.scene_id
		LEFT JOIN subscriber ON service.consumer_id = subscriber.user_name
		WHERE
			service.business_id = 'okPUDtzjrogWsWdsI_Fz39VNbgBc'
		AND service.`handler` = 'okPUDtzjrogWsWdsI_Fz39VNbgBc'
		AND service.`status` = 0
		AND TO_DAYS(NOW()) - TO_DAYS(service.handle_date_time) <= 2
		AND service.is_delete = 0
		AND service.type = 0
		ORDER BY
			handle_date_time DESC**/
		// @formatter:on
		...
		sqlSb.append("	service.scene_id,\n");
		sqlSb.append("	DATE_FORMAT(\n");
		sqlSb.append("		service.handle_date_time,\n");
		sqlSb.append("		'%Y/%m/%d %H:%i:%s'\n");
		sqlSb.append("	) AS handle_date_time,\n");
		sqlSb.append("	UNIX_TIMESTAMP(service.handle_date_time) AS handle_times,\n");
		sqlSb.append("	TIMESTAMPDIFF(\n");
		sqlSb.append("		SECOND,\n");
		sqlSb.append("		service.handle_date_time,\n");
		sqlSb.append("		NOW()\n");
		...
		sqlSb.append("WHERE\n");
		sqlSb.append("	service.business_id = ?\n");
		sqlSb.append("AND service.`handler` = ?\n");
		sqlSb.append("AND service.`status` = 0\n");
		sqlSb.append("AND TO_DAYS(NOW()) - TO_DAYS(service.handle_date_time) <= 2\n");
		sqlSb.append("AND service.is_delete = 0\n");
		sqlSb.append("AND service.type = 0\n");
		sqlSb.append("ORDER BY\n");
		sqlSb.append("	service.handle_date_time DESC\n");
